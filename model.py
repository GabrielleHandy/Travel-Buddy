...
class Destination(db.Model):
    ...
    # planning on creating a function to find the lat/long for this criteria
    
    travel_planner = db.relationship('Travel_planner', back_populates="destination")
    embassy = db.relationship('Embassy', back_populates="destination")
...
class Embassy(db.Model):
    ...
    destination = db.relationship('Destination', back_populates="embassy")
    
    def __repr__(self):
        return f"Embassy Embassy_id:{self.embassy_id} Home Country:{self.home_country} Location:{self.address or self.lat_long}"
...
def connect_to_db(flask_app, db_uri="postgresql:///travelbuddy", echo=True):
    ...
    # test_user=(fname= 'Jane', lname= 'Doe', email='jane@jane.com', password='test', home_country='USA')
